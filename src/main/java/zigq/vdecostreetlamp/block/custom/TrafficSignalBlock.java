...
public class TrafficSignalBlock extends Block implements BlockEntityProvider {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        if (direction.getAxis() == Direction.Axis.Y) {
            direction = ctx.getHorizontalPlayerFacing().getOpposite(); // Keep it horizontal if placed looking up/down
        }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.TRAFFIC_SIGNAL_BLOCK_ENTITY, TrafficSignalBlockEntity::tick);
    }
