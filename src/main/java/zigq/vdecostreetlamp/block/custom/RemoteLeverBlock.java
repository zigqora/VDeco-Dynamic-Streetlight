    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.isClient) {
            ...
            if (newState.get(POWERED)) {
                // Particles are handled by LeverBlock internally on tick/use depending on version, skipping manual call here to avoid access issues.
            }
            ...
            if (toggledCount > 0) {
                player.sendMessage(Text.translatable("block.vdecostreetlamp.remote_lever.success", toggledCount), true);
            } else {
                player.sendMessage(Text.translatable("block.vdecostreetlamp.remote_lever.none_found"), true);
            }
                        ...
                        for (int y = localMinY; y <= localMaxY; y++) {
                            for (int z = chunkMinZ; z <= chunkMaxZ; z++) {
                                ...
                                if (targetState.getBlock() instanceof RedstoneStreetlampHeadBlock) {
                                    boolean isLampRemotelyPowered = targetState.get(RedstoneStreetlampHeadBlock.REMOTELY_POWERED);
                                    if (isLampRemotelyPowered != isLeverOn) {
                                        // Update world block state
                                        world.setBlockState(pos, targetState.with(RedstoneStreetlampHeadBlock.REMOTELY_POWERED, isLeverOn), 3);
