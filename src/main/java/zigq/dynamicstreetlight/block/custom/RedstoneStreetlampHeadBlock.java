...
public class RedstoneStreetlampHeadBlock extends AbstractFlickeringBlock {
    public static final MapCodec<RedstoneStreetlampHeadBlock> CODEC = createCodec(settings -> new RedstoneStreetlampHeadBlock(settings, VoxelShapes.fullCube()));

    @Override
    public MapCodec<? extends RedstoneStreetlampHeadBlock> getCodec() {
        return CODEC;
    }
    public static final BooleanProperty POWERED = Properties.POWERED; // Used to track received power
    public static final BooleanProperty REMOTELY_POWERED = BooleanProperty.of("remotely_powered"); // Track remote
                                                                                                   // toggles
    ...
    public void neighborUpdate(BlockState state, World world, BlockPos pos, Block sourceBlock, BlockPos sourcePos,
        ...
        if (!world.isClient) {
            ...
            if (state.get(POWERED) != isPowered) {
                ...
                // Schedule a tick immediately when power changes so it can start/stop the
                // flicker effect
